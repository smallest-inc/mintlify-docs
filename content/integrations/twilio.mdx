...
description: "Learn how to integrate Smallest AI TTS in Twilio for telephony solutions."
icon: "phone-volume"
---
...
You can access the code for this example on GitHub [here <Icon icon="arrow-up-right-from-square" iconType="solid" />](https://github.com/smallest-inc/waves-examples/tree/main/lightning/telephone_example/twilio_example).
...
- **Twilio** account to handle calls and text-to-speech.

## Setup Steps

### 1. Configure Environment Variables

Start by creating a `.env` file in your project directory with the necessary API keys. This configuration is essential for interacting with **Smallest.ai** and **Twilio**.
...
2. Expose your FastAPI server for twilio by running:
...
```bash
cd lightning/telephone_example
...
```bash
python twilio_example/twilio_app.py
```
...
```bash
python twilio_example/twilio_make_call.py
```

### 6. Testing the Call

- Once the client script is executed, a call will be placed to the provided number with phonetic audio.
- twilio will handle the audio and make the call based on the provided parameters.
...
This public URL will allow external services like twilio to interact with your server.
...
- **API Keys:** Ensure your `.env` file contains all required keys for **twilio** to ensure successful API calls.
...
This setup provides seamless testing of phonetic calls using the **Smallest AI API** and **twilio** for real-time voice interaction.