...
description: 'Stream speech for given text using the Lightning WebSocket API'
hideApiMarker: False
---

## WebSocket API

The Lightning WebSocket API allows you to stream text-to-speech audio in real-time. This is particularly useful for applications requiring low-latency audio generation.
...
const ws = new WebSocket(
  'wss://api.smallest.ai/api/v1/lightning/stream'
);
```
...
<WebSocketPlayground
  endpoint='wss://api.smallest.ai/api/v1/lightning/stream'
  method='WebSocket'
  requestFormat='json'
  defaultBody={{
...
const ws = new WebSocket(
  'wss://waves-api.smallest.ai/api/v1/lightning/get_speech/stream',
  {
    headers: {
      Authorization: 'Bearer YOUR_API_KEY'
    }
